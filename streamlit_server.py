...
# 强制设置环境变量（服务器部署专用）
os.environ['POLYGON_API_KEY'] = 'l7eyBqnxp9XsobMxIBIVHx69zqRlY5rc'
...
# Environment check with forced API key
env_status = []
...
# Force API key configuration for server deployment
polygon_key = os.getenv('POLYGON_API_KEY', 'l7eyBqnxp9XsobMxIBIVHx69zqRlY5rc')
os.environ['POLYGON_API_KEY'] = polygon_key

env_status.append(f"✅ Polygon API Key configured (Server Mode: {polygon_key[:10]}...)")
...
if predict_button and selected_ticker and modules_ok:
    ...
    with st.spinner("🔄 Analyzing stock data and making predictions..."):
        try:
            # Force set global variables for server deployment
            import builtins
            import torch
...
st.markdown("**🔧 Server Mode**: API keys are hardcoded for reliable server deployment.")
